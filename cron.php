...
"DELETE FROM files
WHERE DATEDIFF(CURDATE(), file_date) > 1 AND file_usr != 3");


/*$anonymousfiles = mysqli_query($bdd,
"SELECT * FROM files
INNER JOIN users ON files.file_usr=users.id_usr
WHERE TIMESTAMPDIFF(minute, file_date, CURRENT_DATE) > 10 AND users.id_usr = 3");
...
"DELETE FROM files
WHERE TIMESTAMPDIFF(minute, file_date, CURRENT_DATE) > 10 AND file_usr = 3");
